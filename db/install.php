...
/**
 * Course Activities block installation.
 *
...
defined('MOODLE_INTERNAL') || die();
 ...
 /**
  * Add the course activities block to the dashboard for all users by default
  * when it is installed.
  */
function xmldb_block_course_activities_install() {
    global $DB;

    if ($DB->count_records('block_instances') < 1) {
        // Only add the course activities block if it's being installed on an existing site.
    ...
    $page->blocks->add_block('course_activities', BLOCK_POS_RIGHT, 0, false, 'my-index', $subpagepattern);
    ...
    // course activities block without the admin resetting all of the dashboards.
    //
    // Instead we'll just add the course activities block to their dashboards here. We will only
    // add the course activities block if they still have the myoverview block.
    ...
    foreach ($existingrecords as $existingrecord) {
        ...
        if (isset($seencontexts[$parentcontextid])) {
            ...
            // to add duplicate course activities blocks to the same context. This happens
